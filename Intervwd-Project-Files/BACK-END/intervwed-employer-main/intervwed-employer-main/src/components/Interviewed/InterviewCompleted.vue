    ...
    <v-card-text>
      ...
      >
        ...
        <template v-slot:[`item.interviewed_user_name`]="{ item }">
          <div class="font-weight-medium">{{ item.interviewed_user_name }}</div>
          <div class="text-caption text-medium-emphasis">
        ...
        <template v-slot:[`item.interview_ratings`]="{ item }">
          <div class="d-flex align-center" v-if="item.interview_ratings">
            <v-icon size="small" :color="getRatingColor(item.interview_ratings)"
              >mdi-star</v-icon
            >
            <span
              :class="[
                'ml-1 font-weight-bold',
                getRatingColorClass(item.interview_ratings),
              ]"
            >
              {{ item.interview_ratings.toFixed(1) }}
            </span>
          </div>
          <div else>-</div>
        </template>
        ...
        <template v-slot:expanded-row="{ item }">
          <tr>
            <td
              ...
              style="padding: 10; border: none; margin-left: -20px"
            >
              <v-card
                class="ma-4 pa-4"
                elevation="0"
                ...
                <v-row>
                  ...
                  <v-col cols="12">
                    <v-card elevation="0" class="pa-4 border">
                      ...
                      <div
                        v-if="formatTranscript(item.transcript_text).length > 0"
...
<script>
import { ListOrganizationOverAllInterviews } from "@/graphql/queries.js";
import { generateClient } from "aws-amplify/api";

const client = generateClient();

export default {
  name: "CompletedInterviewsDashboard",
  data() {
    return {
      search: "",
      expanded: [],
      responseArray: [],
      headers: [
        { title: "JOB", key: "job_title", sortable: false },
        { title: "CANDIDATE", key: "interviewed_user_name", sortable: false },
        { title: "DATE", key: "interview_scheduled_on", sortable: false },
        {
          title: "DURATION",
          key: "interview_duration_seconds",
          sortable: false,
        },
        { title: "RATING", key: "interview_ratings", sortable: false },
    ...
    async fetchInterviews() {
      try {
        const result = await client.graphql({
          query: ListOrganizationOverAllInterviews,
          variables: {
            input: {
        ...
        const response = JSON.parse(
          result.data.ListOrganizationOverAllInterviews
        );
