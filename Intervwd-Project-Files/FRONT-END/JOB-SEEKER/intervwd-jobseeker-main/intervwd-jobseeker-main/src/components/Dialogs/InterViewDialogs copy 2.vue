...
export default {
  ...
  methods: {
    ...
    // //     this.mediaRecorder.start(); // no interval → collects continuously
    ...
    //     console.log("✅ Audio ready:", this.recordedFile);
    ...
    // //   // ✅ Just stop the interval, don't reset the value
    ...
    //         this.recordedAudio = new File(
    //           [audioBlob],
    //           `interview_${Date.now()}.webm`,
    //           { type: "audio/webm" }
    //         );

    //         console.log("✅ Recording stopped:", this.recordedAudio);
    ...
    // },
    async getPromptsdetails() {
      // Ensure this method is fast, as it blocks the start of the interview.
      this.InterViewDetails = await this.CreateInterviewMethod(
        this.ActionObj.jd_id
      );
    },
    ...
    async startRecording() {
      ...
      this.mediaRecorder.onstop = () => {
        // This runs AFTER mediaRecorder.stop() is called
    ...
    async endInterview() {
      // This is now called via mediaRecorder.onstop
      ...
      try {
        ...
        console.log("✅ Interview ended and uploaded:", UploadedResult);
        // Optionally emit a success event or show a notification
        this.SnackBarComponent = {
          SnackbarVmodel: true,
          SnackbarColor: "green",
    ...
    handleIframeMessage(event) {
      ...
      if (event.data.action === "interviewCompleted") {
        // If the iframe signals completion, we treat it as a stop event
    ...
    InterViewDialogEmit(Toggle) {
      // Ensure all processes stop when closing the dialog
      this.stopTimer();
      this.stopRecording();
      this.isInterviewing = false;
      this.confirmStop = false;
